...
export default function ManagerBeritaPage() {
  ...
  function handleSave(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    if (editItem) {
      ...
      setItems([{
        id: Date.now(), title: form.title.trim(), excerpt: form.excerpt.trim(),
        content: form.excerpt.trim(), category: form.category,
        date: new Date().toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }),
        bgColor: "bg-gray-100", emoji: "", imageUrl: form.imageUrl || undefined,
      }, ...items]);
    }
    setShowModal(false);
  }
        ...
        {items.map((item) => (
          <div key={item.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-start gap-4">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={() => openEdit(item)} className="p-2 text-gray-400 hover:text-[#2F855A] hover:bg-green-50 rounded-lg transition-colors">
              ...
              {deleteConfirm === item.id ? (
                ...
                <button onClick={() => setDeleteConfirm(item.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                {form.imageUrl && (
                  <button type="button" onClick={() => setForm((f) => ({ ...f, imageUrl: "" }))} className="mt-1.5 text-xs text-red-500 hover:underline">Hapus gambar</button>
                )}
              </div>
              <div>
              ...
              <div>
                ...
                <input value={form.title}
                  onChange={(e) => { setForm({ ...form, title: e.target.value }); setFormErr((er) => ({ ...er, title: undefined as unknown as string })); }}
              ...
              <div>
                ...
                <textarea value={form.excerpt}
                  onChange={(e) => { setForm({ ...form, excerpt: e.target.value }); setFormErr((er) => ({ ...er, excerpt: undefined as unknown as string })); }}
