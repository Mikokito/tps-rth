...
import { getManajerNotifikasi, type NotifItem } from "@/app/actions/notifikasi";
...
export default function ManagerNotifikasiPage() {
  const [items, setItems] = useState<NotifItem[]>([]);
  const [ready, setReady] = useState(false);

  useEffect(() => {
    async function load() {
      const { data } = await getManajerNotifikasi();
      setItems(data);
      setReady(true);
    }
    load();
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-sm text-gray-500">Pengajuan izin/cuti yang menunggu konfirmasi</p>
      </div>
