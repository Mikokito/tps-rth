...
import { ArrowRight, ChevronDown, ChevronUp, UserPlus, FileText, MessageSquare } from "lucide-react";
    ...
    <>
            ...
            {steps.map((s, idx) => (
              <div key={s.num} className="flex gap-5">
                ...
                <div className="flex flex-col items-center">
                  <div className="w-12 h-12 bg-[#2F855A] rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                    {s.num}
                  </div>
                  {idx < steps.length - 1 && (
                    <div className="w-0.5 flex-1 bg-green-200 mt-2 mb-0 min-h-[24px]" />
                  )}
                </div>
                {/* Content */}
      ...
      <section className="py-14 bg-[#F0FFF4]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Siap Bergabung?</h2>
          <p className="text-gray-600 mb-8">
            Pilih salah satu cara di bawah ini untuk memulai perjalanan Anda bersama TPS RTH Cikaret.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href={tpsInfo.gformUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 bg-[#2F855A] text-white font-semibold px-7 py-4 rounded-xl hover:bg-[#276749] transition-colors shadow-lg"
            >
              <FileText className="w-5 h-5" />
              Isi Formulir Pendaftaran
            </a>
            <a
              href={`https://wa.me/${tpsInfo.contact.whatsapp}?text=${encodeURIComponent("Halo, saya ingin mendaftar sebagai nasabah TPS RTH Cikaret.")}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 bg-green-500 text-white font-semibold px-7 py-4 rounded-xl hover:bg-green-600 transition-colors shadow-lg"
            >
          ...
          <div className="mt-6 flex items-center justify-center gap-2 text-sm text-gray-500">
            ...
            <span>Sudah punya akun? <Link href="/kontak" className="text-[#2F855A] font-semibold hover:underline">Hubungi admin</Link></span>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">Persyaratan Pendaftaran</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={item.req} className="flex gap-3 items-start bg-gray-50 rounded-xl p-4 border border-gray-100">
            ...
            {faqs.map((faq, idx) => (
              <div key={idx} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                ...
                >
                  ...
                  {openFaq === idx ? (
                    <ChevronUp className="w-5 h-5 text-[#2F855A] flex-shrink-0" />
                  ) : (
