...
type FormState = { nama: string; hp: string; alamat: string; rw: string; rt: string };
...
export default function UserAkunPage() {
  ...
  const [form, setForm] = useState<FormState>({ nama: "", hp: "", alamat: "", rw: "", rt: "" });
  ...
  useEffect(() => {
    getSession().then((s) => {
      if (s) {
        setSession(s);
        setForm({ nama: s.nama, hp: s.hp, alamat: s.alamat, rw: s.rw ?? "", rt: s.rt ?? "" });
  ...
  async function handleSave(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const { user, error } = await updateProfile({
      ...
      rw: form.rw.trim(), rt: form.rt.trim(), avatar_url: avatar || undefined,
    });
          ...
          <div className="flex flex-col sm:flex-row gap-0">
            ...
            <div className="flex flex-col items-center justify-start gap-3 px-8 py-8 sm:border-r border-b sm:border-b-0 border-gray-100 sm:min-w-52">
              ...
              >
                <div className="w-28 h-28 rounded-full bg-[#2F855A] flex items-center justify-center overflow-hidden ring-4 ring-green-100">
                  {avatar
                    ...
                    : <span className="text-4xl font-bold text-white">{session?.nama?.[0] ?? "U"}</span>
                  }
                </div>
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-400 mt-0.5">Nasabah</p>
              </div>
            </div>
            ...
            <div className="flex-1 p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.rw} onChange={update("rw")} placeholder="01" maxLength={3}
                ...
                <div>
                  ...
                  <input value={form.rt} onChange={update("rt")} placeholder="01" maxLength={3}
                    className="w-full border border-gray-200 bg-white rounded-xl px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#2F855A] focus:border-transparent transition-shadow" />
                </div>
              </div>
