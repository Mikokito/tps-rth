...
import { getNasabahNotifikasi, type NotifItem } from "@/app/actions/notifikasi";
...
export default function UserNotifikasiPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await getNasabahNotifikasi(session.id, session.email);
      setItems(data);
      setReady(true);
    }
    load();
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-sm text-gray-500">Update status pengajuan iuran Anda</p>
      </div>
