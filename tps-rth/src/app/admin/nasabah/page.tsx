...
export default function NasabahPage() {
  ...
  const filtered = useMemo(() => {
    return data.filter((m) => {
      ...
      if (search && !m.nama.toLowerCase().includes(search.toLowerCase()) &&
          !(m.alamat ?? "").toLowerCase().includes(search.toLowerCase())) return false;
      return true;
    });
  ...
  function validate(): FormErrors {
    ...
    if (!form.hp.trim()) errs.hp = "No. HP wajib diisi";
    return errs;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          ...
          <p className="text-2xl font-bold text-gray-400 mt-1">{filtered.length - aktifCount}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Alamat</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">No. HP / WA</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Tgl. Bergabung</th>
        ...
        <div className="md:hidden divide-y divide-gray-50">
          {filtered.length === 0 ? (
            <p className="px-4 py-10 text-center text-sm text-gray-400">Tidak ada nasabah yang sesuai.</p>
          ) : filtered.map((m) => (
            <div key={m.id} className="px-4 py-4">
              <div className="flex items-start justify-between gap-3 mb-1">
                <div>
                  <p className="text-sm font-semibold text-gray-900">{m.nama}</p>
              ...
              {(["nama", "email", "hp"] as const).map((field) => (
                <div key={field}>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">
                    {field === "hp" ? "No. HP / WhatsApp" : field === "email" ? "Email" : "Nama Lengkap"} <span className="text-red-500">*</span>
                  </label>
                  <input
                    value={form[field]}
                    onChange={upd(field)}
                    placeholder={field === "email" ? "email@contoh.com" : field === "hp" ? "08xx-xxxx-xxxx" : ""}
