...
export default function AdminAbsenPage() {
  const now  = new Date();
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        if (session) setAdminNama(session.nama);
        await loadAll();
      } catch (err) {
  ...
  async function handleKonfirmasi(record: AbsenRow, newStatus: "hadir" | "tidak_hadir") {
    ...
    try {
      ...
      const { data } = await supabase
        .from("absensi")
        .update({ status: newStatus, confirmed_at: new Date().toISOString(), confirmed_by: adminNama || "Admin" })
        .eq("id", record.id)
        .select("id, tanggal, staff_id, status, submitted_at, confirmed_at, confirmed_by")
        .single();
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">
                    <span className="sm:hidden">Total Kehadiran</span>
